...
# This is a modified version of the link_clustering program
# that should run with python 3.5. It also ships with a 
# wrapper that allows use with networkx graphs
# Modified: 2016-10-16
# Nathaniel Rodriguez
...
def write_edge2cid(e2c,filename,delimiter="\t"):
    ...
    for e,c in sorted(e2c.items(), key=itemgetter(1)):
        f.write( "{:s}{:s}{:s}{:s}{:s}\n".format(str(e[0]),delimiter,str(e[1]),delimiter,str(c2c[c])) )
    f.close()
    ...
    for cid in sorted(cid2edges.keys()):
        ...
        edges  = ["{:s},{:s}".format(str(ni),str(nj)) for ni,nj in cid2edges[cid]]
